...
login_failure_keywords = [b"incorrect", b"login failed", b"invalid", b"try again"]

# Load in the list of default username and passwords to try out
with open("wordlists/default_passwords.txt", 'r') as opn:
    ...
    for l in br.forms(): 
        for i in l.controls: 
            for k in username_keywords: 
                if (i.name and k in i.name.lower()) or (i.id and k in i.id.lower()): # IS TO GO
                    login_form = l 
                    username_control = i 

            for k in password_keywords:
                if (i.name and k in i.name.lower()) or (i.id and k in i.id.lower()) and not k == username_control:
                    login_form = l
                    password_control = i
...
def try_username_password(br, user_control, pass_control, username, password):
    ...
    ub = bytes(username, encoding='utf-8')
    uu = bytes(username, encoding='utf-8')
    s = br.submit().read()
    sub = s.replace(uu, b'').replace(ub, b'')
    return sub
    #return br.submit().read().replace(bytes(username, encoding='utf-8'), b'').replace(bytes(password), b'')
...
def is_page_login_success(page, original_page):
    for i in login_failure_keywords:
        if i.lower() in page.lower() and not i.lower() in original_page.read().lower():
            return False
    return True
...
def main():
    ...
    br.addheaders= [('User-agent', user_agent[0])]
    ...
    if not login_form or not username_control or not password_control:
        ...
        print("It may also be un-loadable without javascript.")
        exit()
